...
| **Data & Model (라우팅 알고리즘)** | OSMnx, NetworkX (A*), GeoPandas, Shapely, Scikit-learn (K-Means 쾌적도 분류) |
...
| 데이터명 | 출처 | 활용 목적 |
| :--- | :--- | :--- |
| **보행자/자전거 도로망 그래프** | OSMnx (OpenStreetMap) | 길찾기 요청 시 bbox 기준으로 실시간 다운로드, A* 라우팅 그래프 구성 |
...
2. OSMnx로 해당 Bbox의 보행 도로망 그래프 실시간 다운로드
3. 시간대별 GeoJSON(109장)으로 각 도로 엣지의 그림자 덮힘 비율 계산 → `shadow_weight` 부여
4. **K-Means 쾌적도 등급 분류:** 엣지별 (그림자비율, 직사광선노출거리, 보도여부) 피처를 StandardScaler 정규화 후 Silhouette Score 기반 최적 K 자동 선택 (K=2~5), 등급별 배율 shadow_weight에 반영
5. **횡단보도 정밀 제약:** Shapely 교차 분석으로 실제 차도 라인 감지 및 무단횡단 패널티 적용
6. **실시간 신호 패널티:** 빨간불 잔여시간을 엣지 가중치에 추가 반영
7. **A* 탐색:** Haversine 기반 분(minute) 단위 휴리스틱으로 그늘 + 쾌적도 + 신호 복합 최적 경로 탐색
