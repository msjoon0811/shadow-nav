...
3. 출발지/도착지 입력칸, '자전거 이용자 전용' 체크박스, **시간대 조절 슬라이더(09:00 ~ 18:00)** UI를 만듭니다. (최초 접속 시 **사용자의 현재 시스템 시간**을 기본값으로 로딩하고, 슬라이더로 변경할 수 있도록 사용자 친화적으로 구현)
...
2. **나/팀장 (데이터 모델 & 머신러닝):** 
   ...
   - 쾌적도가 반영된 도로 위에서 A* 알고리즘을 가동하여 가장 시원한 길을 찾아 `app.py`로 리턴해 줍니다.
...
5. 이 점수판 위에 A*(A-Star) 라우팅 알고리즘을 굴립니다. 목적지 방향성을 고려해 탐색 속도를 높이고 신호 대기시간이나 땡볕 언덕길은 패널티를 주고, 그늘진 평지는 가산점을 줍니다.
